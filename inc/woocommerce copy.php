...
// Adds a simple "Badge" text field to each product in the WooCommerce admin.
...
// Remove unwanted checkout fields to keep the form clean
add_filter( 'woocommerce_checkout_fields', function ( $fields ) {
	// Remove company field from billing (optional — uncomment if needed)
	// unset( $fields['billing']['billing_company'] );
	return $fields;
} );


// ─── Single Product Summary: Title & Price ──────────────────────────────
// Remove WC's default title (priority 5) and price (priority 10) callbacks
// and replace with our own styled output so the template doesn't need to
// hardcode them.
...
// Without zoom, the gallery <a> click falls through to PhotoSwipe (lightbox)
// directly. No filter needed — WC 10.7 no longer renders a trigger icon in PHP.
...
// ─── Single Product: Display Size Attribute Only ─────────────────────────
add_action( 'woocommerce_single_product_summary', function () {
	global $product;
	...
	echo '<div class="single-product__attribute single-product__attribute--size">';
	echo '<span class="single-product__attribute-label">' . esc_html__( 'Size:', 'doubletap' ) . '</span> ';
	echo '<span class="single-product__attribute-value">' . esc_html( $size ) . '</span>';
	echo '</div>';
}, 11 );