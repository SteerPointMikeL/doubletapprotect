<?php
/**
 * Universal Section: Safety Information
 *
 * ACF flexible content layout: safety_information (field: content_sections)
 * (Formerly the page-locked "info_safety" layout on the Information page.)
 *
 * Fields:
 *   label, heading (text, optional)
 *   section_background (select: bg | surface)
 *   body (wysiwyg)
 *   sds_files (repeater: product_name, file)
 *   cta_label / cta_url (fallback CTA if no files are added)
 *
 * @package doubletap
 */
			...
			<?php if ( $sds_files ) : ?>
				<ul class="sds-list" role="list">
					<?php foreach ( $sds_files as $entry ) :
						$file = ! empty( $entry['file'] ) ? $entry['file'] : null;
						$name = ! empty( $entry['product_name'] ) ? $entry['product_name'] : '';
						if ( ! $file ) { continue; }
