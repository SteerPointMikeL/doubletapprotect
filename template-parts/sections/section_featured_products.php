 ...
 *
 * ACF flexible content layout: section_featured_products
 * Pulls products by ACF-configured IDs or falls back to handoff defaults.
 *
 * @package doubletap
 */
...
$get            = function_exists( 'get_sub_field' ) && get_sub_field( 'featured_products_heading' ) !== null ? 'get_sub_field' : 'get_field';
$heading        = $get( 'featured_products_heading' )     ?: 'Featured Products';
$label          = $get( 'featured_products_label' )       ?: 'Best Sellers';
$desc           = $get( 'featured_products_description' ) ?: '';
$product_ids    = $get( 'featured_product_ids' );         // relationship or post_object field

// product_ids comes from ACF relationship field only
...
if ( empty( $ids ) ) {
	// Final fallback: most recent 3 published products
	$ids = get_posts( [
		'post_type'      => 'product',
		'post_status'    => 'publish',
		'posts_per_page' => 3,
...
if ( empty( $ids ) ) {
	return; // No products yet — show nothing
}
?>
